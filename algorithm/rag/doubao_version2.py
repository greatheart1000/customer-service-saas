...
def summarize_and_extract_keywords(text):
    ...
    "messages": [
        ...
        {
            "role": "user",
            "content": summarize_and_extract_keywords_prompt.replace('[text]',text)  }
    ]
}
    response = requests.post(url, headers=headers, data=json.dumps(data))
    # 打印响应内容
    result =response.json()['choices'][0]['message']['content']
    print('[step0] 提取文本中的摘要和关键词:',result)
    return result
...
extracted_summarize_and_keywords =  summarize_and_extract_keywords(text)

result = create_base_extracted_entities(text=text)

summarized_entities= create_summarized_entities(result)

relationships_entity = create_base_entity_graph(summarized_entities)
