...
def main():
    ...
    batch_size = 512
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of workers ，8类
    ...
    net = resnet50(num_classes=len(laji_list))
    net.to(device)
    print(">>> Loading weights from ./resnet50_100.pth")
    sd = torch.load('resnet50_100.pth', map_location=device)
